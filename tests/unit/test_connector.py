...
from google.cloud.alloydbconnector.connector import _dial_phase
from google.cloud.alloydbconnector.connector import _tag_dial_phase
...
@pytest.mark.parametrize(
    ...
    [
        (ConnectionRefusedError(111, "nope"), DIAL_TCP_ERROR),
        (ssl.SSLError("bad handshake"), DIAL_TLS_ERROR),
        (RuntimeError("short read"), DIAL_MDX_ERROR),
    ],
)
def test_connect_records_dial_error_status(
    ...
    """Each phase metadata_exchange tags reports its own status, once. The
    tag travels on the exception itself, so it has to survive the hop back
    from the executor thread that ran the dial."""
    _tag_dial_phase(error, expected_status)
    connector, mr = _connector_with_recorder(credentials, fake_client)
    with connector:
...
def test_metadata_exchange_tags_errors_past_the_handshake(
    credentials: FakeCredentials, error: Exception
) -> None:
    """Any failure past the TLS handshake belongs to the metadata exchange.
    Leaving it untagged would record it as a cache error instead."""
        ...
        with patch("socket.create_connection", return_value=MagicMock()):
            ...
            with patch.object(Connector, "_metadata_exchange", side_effect=error):
                with pytest.raises(type(error)) as exc_info:
                    connector.metadata_exchange(INSTANCE_URI, "127.0.0.1", ctx, False)
    assert _dial_phase(exc_info.value, default="") == DIAL_MDX_ERROR
...
def test_metadata_exchange_tags_tcp_errors(
    credentials: FakeCredentials,
) -> None:
    """A failure to reach the server is tagged as the TCP phase. Tagging
    rather than wrapping leaves the error callers see untouched: it is still
    a ConnectionRefusedError, and it still carries its errno."""
    with Connector(credentials) as connector:
        with patch(
    ...
    assert _dial_phase(exc_info.value, default="") == DIAL_TCP_ERROR
    assert exc_info.value.errno == 111


def test_metadata_exchange_tags_tls_errors(
    credentials: FakeCredentials,
) -> None:
    """A handshake failure is tagged as the TLS phase and stays an SSLError."""
    with Connector(credentials) as connector:
        with patch("socket.create_connection", return_value=MagicMock()):
            ctx = MagicMock()
            ...
            with pytest.raises(ssl.SSLError) as exc_info:
                connector.metadata_exchange(INSTANCE_URI, "127.0.0.1", ctx, False)
    assert _dial_phase(exc_info.value, default="") == DIAL_TLS_ERROR
